...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var ifrCalls = document.createElement("iframe"); 
		ifrCalls.style.display = "none";
		document.body.appendChild(ifrCalls);
		...
		callUrl = function (url) {
		    ifrCalls.src = url + "?page=" + ts.p.postData.page + "&rows=" + ts.p.postData.rows + "&sidx=" + ts.p.postData.sidx + "&sord=" + ts.p.postData.sord;
		},
		...
		$.extend(grid, { populate: populate, emptyRows: emptyRows, beginReq: beginReq, endReq: endReq, callUrl: callUrl });
		this.grid = grid;
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.ajaxCalls != null) {
				    try {
                        //  DynamicIcon Class, to create structure and add click event
				        var DynamicIcon = function () {
				            this.td = "";

				            this.Create = function (elemids, title, url, icon) {
				                this.td = $("<td class='ui-pg-button ui-corner-all'></td>");
				                this.td.append("<div class='ui-pg-div'><span class='ui-icon " + icon + "'></span></div>");

				                this.td.attr({ "title": title || "", id: title.replace(/\s+/g, '') + "_" + elemids }).click(function (e) {
				                    e.preventDefault();
				                    if (!$(this).hasClass('ui-state-disabled')) {
				                        $t.grid.callUrl(url);
				                    }
                        ...
                        //  Create each Icon
				        for (var i = 0; i < o.ajaxCalls.length; i++) {
				            var item = o.ajaxCalls[i];
				            if (item.icon === undefined || item.icon === "") {
				                item.icon = "ui-icon-note";
				            }
				            var tmpDIcon = new DynamicIcon();
				            tmpDIcon.Create(elemids, item.title, item.url, item.icon);
				            $("tr", navtbl).append(tmpDIcon.td);
				        }
                        
				    } catch (e) { console.log(e); console.error("Error: ajaxCalls it's not a JSON Array"); }
				}
